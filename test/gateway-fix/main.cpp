...
void writeFile(const std::filesystem::path& path) {
	constexpr auto vmoptions =
			...
			"-javaagent:C:\\Program Files\\JetBrains\\ja-netfilter-all\\ja-netfilter.jar=jetbrains\n";
...
bool __stdcall DllMain(void* instance, std::uint32_t reason, void*) {
	...
	if (auto thread = CreateThread(nullptr, 0, [](void* data) -> unsigned long {
		MH_Initialize();
		MH_CreateHook((LPVOID) CreateProcessW, (LPVOID) createProcessWHook, (void**)&CreateProcessWOriginal);
		MH_EnableHook((LPVOID) CreateProcessW);
		return 0;
	}, instance, 0, nullptr))
		CloseHandle(thread);
